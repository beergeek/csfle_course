...
public class App {
    ...
    public boolean trashEmployeeKey(String connectionString, 
            Map<String, Map<String, Object>> kmsProvider, 
            MongoNamespace keyvaultNamespace, 
            UUID employeeKey) {
        try (ClientEncryption clientEncryption = this.getClientEncryption(connectionString, keyvaultNamespace, kmsProvider)) {
            BsonBinary binKey = new BsonBinary(employeeKey);
            ObservableSubscriber<DeleteResult> delSubscriber = new OperationSubscriber<DeleteResult>();
            clientEncryption.deleteKey(binKey).subscribe(delSubscriber);
            DeleteResult delRes = delSubscriber.first();
            if (delRes.getDeletedCount() == 1) {
                return true;
            } else {
                throw new IllegalStateException("Failed to delete DEK");
            }
        }
    }
        ...
        String schemaJson = """
{
    ...
    public static void main( String[] args ) {
        ...
        try (MongoClient secureClient = app.getMdbClient(connectionString, 5000, false, autoEncryptionSettings)) {
            ...
            plainEncryptedColl.find(eq("_id", insertedId))
                .subscribe(encDocSubscriber);
            Document encryptedResult = encDocSubscriber.first();
            ...
            app.trashEmployeeKey(connectionString, kmsProvider, keyvaultNamespace, employeeKeyId);
            ...
            try {
                ObservableSubscriber<Document> postDocSubscriber = new OperationSubscriber<Document>();
                encryptedColl.find(eq("_id", insertedId))
                    .subscribe(postDocSubscriber);
                Document postDecryptedResult = postDocSubscriber.first();
                if (postDecryptedResult != null) {
                    System.out.println(postDecryptedResult.toJson());
                }
            } catch (MongoClientException ee) {
                String eeMsg = ee.getMessage();
                if (eeMsg.contains("not all keys requested were satisfied")) {
                    System.out.println("DEK for use missing or deleted.");
                } else {
                    System.err.print("Decryption err");
                    ee.printStackTrace();
                    System.exit(1);
                }
            }
